package com.practice.Chapter_12_Heap.heap
...
class Heap(val maxSize: Int) {
    ...
    fun toss(key: Int): Boolean {
        ...
        heapArray[currentSize++] = Node(key)
        count++
        return true
    }
    ...
    private fun trickleDown(index: Int) {
        ...
        while (i < currentSize / 2) {
            ...
            smallerChild = if ((rightChild < currentSize &&
            ...
            if (top!!.key <= heapArray[smallerChild]!!.key)
                break
            heapArray[i] = heapArray[smallerChild]
            i = smallerChild
        }
        heapArray[i] = top
    }
